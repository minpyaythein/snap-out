...
Three runtime contexts communicate via `chrome.runtime`/`chrome.tabs` messaging. **`storage.js` is shared** — loaded by the service worker via `importScripts('storage.js')` and by the popup via a `<script>` tag — so its functions (`getSettings`, `getThreshold`, `normalizeHostname`, etc.) are global in both. **`i18n.js` is similarly shared** (the EN/JA dictionary + `t`/`tFormat`), loaded by the **popup** (`<script>` tag) and the **content script** (manifest `content_scripts` list *and* the `chrome.scripting` fallback, which injects `['i18n.js', 'content.js']`); the background doesn't load it — it just forwards the saved `language` in `SHOW_POPUP`.
...
- **`popup/`** — the settings UI (add/remove sites, pick difficulty, set duration). Reads/writes persistent settings via the shared `storage.js` helpers and *reads* `chrome.storage.session` for the live session timer. It never **writes** runtime state directly — counter resets go through the background (`RESET_SITE` / `THRESHOLD_CHANGED`). Tracked sites are capped at `MAX_SITES` (6) in `popup.js`, and the add box validates input is a real domain (`isValidHostname`).
...
- **Two storage areas, different lifetimes**: `chrome.storage.sync` holds persistent *settings* (`trackedSites`, `thresholds`, `difficultyLevel`, `defaultThreshold`, `language`); `chrome.storage.session` holds *runtime state* (`elapsed`, `lastActiveTime`, `activeHostname`, `windowFocused`, `popupShown`). Don't conflate them.
- **`popupShown[hostname]` gates re-display** — once true the popup won't re-show until it's cleared. Flows that *reset* a timer delete the hostname from **both** `elapsed` and `popupShown` — add/remove via the background's `RESET_SITE` handler (which flushes the active site first, so resetting one site never wipes another's running timer), a solved/dismissed challenge via `DISMISS_POPUP`. **Changing the duration does NOT reset elapsed** — it re-evaluates against the new threshold (`THRESHOLD_CHANGED`), only clearing `popupShown`/overlays if the new threshold is now above the accumulated time. `checkThreshold` only ever *sets* `popupShown`, never clears it, so any "the limit went up, take the nudge down" logic lives in the `THRESHOLD_CHANGED` handler.
...
- **Content-script injection fallback**: `sendPopupMessage` first tries `tabs.sendMessage`; if the content script isn't present (e.g. page loaded before install), it injects `content.js`/`content.css` via `chrome.scripting` and retries. It skips pages it can't script (`canInjectInto` — `chrome://`, Web Store, error pages, `file://`) and logs the rest softly. On the page side, `content.js` wraps its `chrome.runtime` calls in `safeSendMessage` so an orphaned script (after an extension reload/update) fails quietly instead of throwing "Extension context invalidated".
- **Multi-tab overlays**: a nudge shows on every tab of the hostname, and solving/dismissing on any one tears them all down (`hideOverlaysForHostname`). `SHOW_POPUP` carries a `force` flag — `true` rebuilds with a fresh problem (initial fire); `false` is a catch-up for tabs that appear/reload after the fire and is ignored by `content.js` if an overlay is already up (so a half-typed answer isn't wiped). It also carries `language` (EN/JA) so the overlay renders in the chosen language without a storage round-trip.
- **Localization is a custom layer, not `chrome.i18n`**: the built-in API keys off the *browser's* UI locale and can't be toggled at runtime, which is why there's an in-app EN/JA selector backed by `i18n.js` + the `language` sync setting instead. Popup static strings use `data-i18n`/`data-i18n-placeholder` (swapped by `applyTranslations`); JS-driven strings (errors, session note, empty row, duration units) go through `t()`/`tFormat()` against a module-level `currentLang`. The math problems are language-neutral; only UI chrome is translated. Host names in overlay copy are still injected via `textContent` inside `fillTemplate` (no innerHTML for user-controlled values).
...
- **Message types** flowing between contexts: `SHOW_POPUP` (+`force`, +`language`), `HIDE_OVERLAY`, `DISMISS_POPUP`, `THRESHOLD_CHANGED`, `RESET_SITE`. Note the popup never writes runtime state (`elapsed`/`popupShown`/`lastActiveTime`) directly — it sends `RESET_SITE`/`THRESHOLD_CHANGED` so the background can flush the active site first and mutate under the `runExclusive` lock. Writing session state from the popup races the background and clobbers the active site's un-flushed time.
