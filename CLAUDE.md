...
- **`background.js`** (service worker) — the timer engine. Owns all time tracking. Tracks the active hostname and accumulated seconds in `chrome.storage.session` (survives SW restarts within a browser session). A recurring 30s alarm plus event listeners (`tabs.onActivated`, `tabs.onUpdated`, `windows.onFocusChanged`) call `checkThreshold()`, which flushes elapsed time and, when the threshold is hit, sends a `SHOW_POPUP` message to the active tab.
...
- **`popupShown[hostname]` gates re-display** — once true the popup won't re-show until elapsed is reset. Any flow that resets a timer (add site, remove site, dismiss, change duration) must delete the hostname from **both** `elapsed` and `popupShown`.
...
- **Message types** flowing between contexts: `SHOW_POPUP`, `HIDE_OVERLAY`, `DISMISS_POPUP`, `RESET_TIMER`.
